...
class StoredRar
{
	...
	public function create($filename)
	{
		...
		$this->writeHeader(0x73, 0x0000, [[0, 2], [0, 4]]);
		return true;
	}
		...
		for ($i = 0; $i < count($parts); $i++) {
			...
			if (!isset($c[$parts[$i]])) {
				...
				$this->writeHeader(0x74, $this->setBits([5, 6, 7, 15]), [
					[0, 4],                    // Packed size = 0 for directories
					[0, 4],                    // Unpacked size = 0 for directories
					[0, 1],                    // Host OS = "MS DOS"
					[0, 4],                    // File CRC = 0 for directories
					[$this->getDateTime(), 4],    // File time = Current date and time in MS DOS format
					[20, 1],                    // RAR version = 2.0
					[0x30, 1],                    // Method = Store
					[strlen($cname), 2],        // Name size
					[0x10, 4],                    // File attributes = Directory
	...
	public function addFile($name, $dir = null)
	{
		...
		$this->writeHeader(0x74, $this->setBits([15]), [
			[$size, 4],                                // Packed size = File size
			[$size, 4],                                // Unpacked size = File size
			[0, 1],                                    // Host OS = "MS DOS"
			[crc32($data), 4],                            // File CRC
			[$this->getDateTime(filemtime($name)), 4],    // File time
			[20, 1],                                    // RAR version = 2.0
			[0x30, 1],                                    // Method = store
			[strlen($fname), 2],                        // Name size
			[0x20, 4],                                    // File attributes = Archived
			$fname,                                            // Filename
		]);
	...
	// обязательных 7 байт.
	private function writeHeader($headType, $headFlags, $data = [])
	{
		if (!in_array($headType, [0x72, 0x73, 0x74])) {
			return false;
		}
		...
		$header = $this->writeBytesToString(array_merge([$headType, [$headFlags, 2], [$headSize, 2]], $data));
