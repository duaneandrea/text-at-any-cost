...
/**
 * Класс для работы с PDF, пока функции просто собраны в кучу
 *
 * Class Pdf
		...
		if (empty($options["Filter"])) {
			...
			foreach ($options as $key => $value) {
				if ($key == "ASCIIHexDecode") {
					$_stream = self::decodeAsciiHex($_stream);
				}
				if ($key == "ASCII85Decode") {
					$_stream = self::decodeAscii85($_stream);
				}
				if ($key == "FlateDecode") {
					$_stream = self::decodeFlate($_stream);
				}
			}
			$data = $_stream;
		}
